...
describe( 'Test DB: Prep Find Data', () => { 
  ...
  it( 'find $in', async () => {
    let cursor = coll.find({ i: { $in : [123,234] } })
    let result = await cursor.toArray()
  ...
  it( 'find $nin', async () => {
    let cursor = coll.find({ i: { $nin : [123,12,23] } })
    let result = await cursor.toArray()
  ...
  it( 'find $like', async () => {
    let cursor = coll.find({ 'sub.str': { $like : 'test' } })
    let result = await cursor.toArray()
    // console.log( 'find $nin', result )
    assert.equal( result.error, undefined )
    assert.equal( result.length, 2 )
  ...
  it( 'find equal $not', async () => {
    let cursor = coll.find({ $not: [ { str: 'test2' }, { i : 234 } ] })
    let result = await cursor.toArray()
    // console.log( 'find $nin', result )
    assert.equal( result.error, undefined )
    assert.equal( result.length, 1 )
  ...
  it( 'find equal $nor', async () => {
    let cursor = coll.find({ $nor: [ { str: 'test2' }, { i : 234 } ] })
    let result = await cursor.toArray()
    // console.log( 'find $nin', result )
    assert.equal( result.error, undefined )
    assert.equal( result.length, 1 )
